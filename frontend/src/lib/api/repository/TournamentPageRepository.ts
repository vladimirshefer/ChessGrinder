    ...
    calculateResults(tournamentData: TournamentPageData) {
        ...
        allMatches.forEach(match => {
            ...
            if (match.result === "WHITE_WIN") {
                if (whiteId) this.computeIfAbsent(pointsMap, whiteId, 1, i => i + 1)
                if (blackId) this.computeIfAbsent(pointsMap, blackId, 0, i => i)
            } else
            if (match.result === "BLACK_WIN") {
                if (blackId) this.computeIfAbsent(pointsMap, blackId, 1, i => i + 1)
                if (whiteId) this.computeIfAbsent(pointsMap, whiteId, 0, i => i)
            } else
            if (match.result === "DRAW") {
                if (whiteId) this.computeIfAbsent(pointsMap, whiteId, 0.5, i => i + 0.5)
                if (blackId) this.computeIfAbsent(pointsMap, blackId, 0.5, i => i + 0.5)
            } else
            if (match.result === "BUY") {
                if (whiteId) this.computeIfAbsent(pointsMap, whiteId, 1, i => i + 1)
                if (blackId) this.computeIfAbsent(pointsMap, blackId, 1, i => i + 1)
            ...
            if (whiteId && blackId) this.computeIfAbsent(enemiesMap, whiteId, new Set<string>([blackId]), enemyPoints => enemyPoints.add(blackId))
            if (whiteId && blackId) this.computeIfAbsent(enemiesMap, blackId, new Set<string>([whiteId]), enemyPoints => enemyPoints.add(whiteId))
        })
